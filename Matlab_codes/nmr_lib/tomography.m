...
%opR - > Operadores da Parte Real  e opI - > Operadores da parte Imaginaria              % 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for k=1:length(tomospin)
      [opR{k} opI{k}] = transitionstomo(nq,tomospin(k));
end
...
for ll =1:length(pul)
        ...
        % Find the opearators that can be observed after the reading pulses
          opRpul{k,ll}  = transitionspul3(opR{k},pul{ll},nq,1);
          opIpul{k,ll}  = transitionspul3(opI{k},pul{ll},nq,0);

        % Create a list of observables. If the number os spins to be tomographed is lower than 
        % the total number of spin, here we find the lines that must the added
          [listobR{k,ll} ssR{k,ll}] = subcompare2sigb(opRpul{k,ll},subbasiss,nq);
          [listobI{k,ll} ssI{k,ll}] = subcompare2sigb(opIpul{k,ll},subbasiss,nq);
        end
...
for ll=1:length(listobR);
    ...
    for k=1:length(tomospin)
       ...
       matelemenR{k,ll} = solveop(specr,listobR{k,ll},ssR{k,ll},length(tomospin));
       matelemenI{k,ll} = solveop(speci,listobI{k,ll},ssI{k,ll},length(tomospin));
...
% Resolve a equa????es para encontrar os coeficientes assocoados aos
% operadores 
