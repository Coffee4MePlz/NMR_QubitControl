function [fid, atempo] = sim_fid2(roh,T2);
% function [fid, atempo] = sim_fid2(roh,T2)
% Simula o fid com decaimento exponecial 1/T2 medindo no segundo qubit
% fid -> intencidade do sinal medido
% atempo -> pontos de aquisicao no dominio do tempo
% sempre medindo no qubit B no tx
...
for i = 1:Np
    ...
    aux = PartialTrace(roh,1); % estado reduzido qubit B
...
%end of function [fid, atempo] = sim_fid2(roh,T2)