function [U, roh] = shapedpulse_Par(shape_obs,pw_obs,phase_obs,ramp_obs,power_obs,roha, L_Spec, L_Mol,channel)
...
for k=1:np 
Hrf= 0;
   Hrf = Hrf + wr_obs(k)*(cos(ph_obs(k))*L_Mol(:,:,(3+2*chn)) + sin(ph_obs(k))*L_Mol(:,:,(4+2*chn)));
