function [U, roh] = shapedpulse_Hom(shape_obs,pw_obs,phase_obs,ramp_obs,power_obs,roha)%, L_Spec, L_Mol)
%(shape_obs,shape_dec,pw_obs,pw_dec,phase_obs,phase_dec,... 
    %ramp_obs,ramp_dec,power_obs,power_dec,roha) %, L_Spec, L_Mol) % future inputs for when 
    % parallelizing and when developing for dec and obs
...
Hrf= 0;
   ...
   %HARDCODED FOR TRIFLUOR1
   % not parallelized
   for j=1:mol.nspin()
        Hrf = Hrf + wr_obs(k)*(cos(ph_obs(k))*mol.Ix{j} + sin(ph_obs(k))*mol.Iy{j});
   end
          
   %parallelizing:
          %wr_obs(k)*(cos(ph_obs(k))*L_Mol(:,:,1) + sin(ph_obs(k))*L_Mol(:,:,2))... 
          %+ wr_dec(k)*(cos(ph_dec(k))*L_Mol(:,:,2) + sin(ph_dec(k))*L_Mol(:,:,4)); %, L_Spec, L_Mol) 
