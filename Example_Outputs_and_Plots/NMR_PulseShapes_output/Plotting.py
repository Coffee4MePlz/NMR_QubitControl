...
dataname = input("Enter the file name: ")

if not dataname:
    print("No file name provided. Please make sure the file ends with .RF \n ... Exiting program.")
else:
    data = np.loadtxt(dataname, comments="#", usecols=(0, 1)) #
    #data = np.loadtxt('Shape_(Roty(90)Xid)_PW-100NP-100_SA8SP14__Opt1_Obs.RF', comments="#", usecols=(0, 1)) # for actual data
    ...
    ax1.set_title('Phase Obs')
    ax2.set_title('Amplitude Obs')
    ax1.set_ylim([0, 360])
    ax2.set_ylim([0, 1124])
    ax2.set_xlabel('Pulse point number/timestep (pp/us)')
